...
type MutasiPersediaan interface {
	...
	FindByCriteria(ctx *abstraction.Context, filter *model.FilterData) (data *model.MutasiPersediaanEntityModel, err error)
...
func (r *mutasipersediaan) FindByCriteria(ctx *abstraction.Context, filter *model.FilterData) (data *model.MutasiPersediaanEntityModel, err error) {
	conn := r.CheckTrx(ctx)
	query := conn.Model(&model.MutasiPersediaanEntityModel{})
