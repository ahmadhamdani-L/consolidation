...
type Export interface {
	GetData(ctx *abstraction.Context, m model.ExportFilter, tipe string) ([]interface{}, error)
}
...
func (r *export) GetData(ctx *abstraction.Context, m model.ExportFilter, tipe string) ([]interface{}, error) {
	conn := r.CheckTrx(ctx)
	var datas []interface{}
	...
	query := conn.Table("? tbm", tableMain).
		...
		Where("aup.company_id = ? AND aup.period = ? AND aup.version = ?", m.CompanyID, m.Period, m.Version).
