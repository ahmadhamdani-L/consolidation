...
type EmployeeBenefit interface {
	...
	FindByCriteria(ctx *abstraction.Context, filter *model.FilterData) (data *model.EmployeeBenefitEntityModel, err error)
...
func (r *employeebenefit) FindByCriteria(ctx *abstraction.Context, filter *model.FilterData) (data *model.EmployeeBenefitEntityModel, err error) {
	conn := r.CheckTrx(ctx)
	query := conn.Model(&model.EmployeeBenefitEntityModel{})
