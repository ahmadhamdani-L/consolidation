...
func (r *coagroup) Find(ctx *abstraction.Context, m *model.CoaGroupFilterModel, p *abstraction.Pagination) (*[]model.CoaGroupEntityModel, *abstraction.PaginationInfo, error) {
	...
	//filter
	tableName := model.CoaGroupEntityModel{}.TableName()
	...
	if err := query.Preload("UserCreated").Preload("UserModified").Preload("CoaType").Find(&datas).WithContext(ctx.Request().Context()).Error; err != nil {
...
func (r *coagroup) FindByID(ctx *abstraction.Context, id *int) (*model.CoaGroupEntityModel, error) {
	...
	if err := conn.Where("id = ?", &id).Preload("CoaType").Preload("UserCreated").Preload("UserModified").First(&data).WithContext(ctx.Request().Context()).Error; err != nil {
		return &data, err
	}
