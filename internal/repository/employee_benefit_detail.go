...
type EmployeeBenefitDetail interface {
	...
	FindWithCodes(ctx *abstraction.Context, fmbID *int, code *string) (*model.EmployeeBenefitDetailEntityModel, error)
}
...
func (r *employeebenefitdetail) FindWithCodes(ctx *abstraction.Context, fmbID *int, code *string) (*model.EmployeeBenefitDetailEntityModel, error) {
	conn := r.CheckTrx(ctx)

	var data model.EmployeeBenefitDetailEntityModel
	tmp := fmt.Sprintf("%s", *code)
	if err := conn.Where("formatter_bridges_id = ? AND code ILIKE ?", fmbID, tmp+"%").First(&data).Error; err != nil {
		return &data, err
	}
	return &data, nil
}
