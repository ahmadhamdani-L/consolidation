...
import (
	...
	"worker-validation/internal/app/validation"
...
func NewConsumer(f *factory.Factory) {
	...
	for run == true {
		...
		default:
			...
			if err = json.Unmarshal([]byte(ev.Value), &data); err != nil {
				fmt.Printf("Error unmarshalling. Error: %s", err.Error())
			}
			if strings.Trim(*ev.TopicPartition.Topic, " ") == "VALIDATION" {
				// trialbalance.NewHandler(f).Action(string(ev.Key), data)
				validation.NewHandler(f).Action(string(ev.Key), data)
			}
		}
	}
