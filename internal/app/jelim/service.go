...
type Service interface {
	Find(ctx *abstraction.Context, payload *dto.JelimGetRequest) (*dto.JelimGetResponse, error)
	FindByID(ctx *abstraction.Context, payload *dto.JelimGetByIDRequest) (*dto.JelimGetByIDResponse, error)
	Create(ctx *abstraction.Context, payload *dto.JelimCreateRequest) (*dto.JelimCreateResponse, error)
	Update(ctx *abstraction.Context, payload *dto.JelimUpdateRequest) (*dto.JelimUpdateResponse, error)
	Delete(ctx *abstraction.Context, payload *dto.JelimDeleteRequest) (*dto.JelimDeleteResponse, error)
	Export(ctx *abstraction.Context, payload *dto.JelimExportRequest) (*dto.JelimExportResponse, error)
	GetVersion(ctx *abstraction.Context, payload *dto.GetVersionRequest) (*dto.GetVersionResponse, error)
}

func NewService(f *factory.Factory) *service {
	repository := f.JelimRepository
	JelimDetailRepository := f.JelimDetailRepository
	JpmRepository := f.JpmRepository

	db := f.Db
	return &service{
		Repository:            repository,
		JelimDetailRepository: JelimDetailRepository,
		JpmRepository:         JpmRepository,
		Db:                    db,
	}
}

func (s *service) Find(ctx *abstraction.Context, payload *dto.JelimGetRequest) (*dto.JelimGetResponse, error) {
	data, info, err := s.Repository.Find(ctx, &payload.JelimFilterModel, &payload.Pagination)
	if err != nil {
		return &dto.JelimGetResponse{}, response.ErrorBuilder(&response.ErrorConstant.InternalServerError, err)
	}
	result := &dto.JelimGetResponse{
		Datas:          *data,
		PaginationInfo: *info,
	}
	return result, nil
...
func (s *service) FindByID(ctx *abstraction.Context, payload *dto.JelimGetByIDRequest) (*dto.JelimGetByIDResponse, error) {
	data, err := s.Repository.FindByID(ctx, &payload.ID)
	if err != nil {
		return &dto.JelimGetByIDResponse{}, response.ErrorBuilder(&response.ErrorConstant.InternalServerError, err)
	}
	result := &dto.JelimGetByIDResponse{
		JelimEntityModel: *data,
	}
	return result, nil
}
func (s *service) Create(ctx *abstraction.Context, payload *dto.JelimCreateRequest) (*dto.JelimCreateResponse, error) {
	var data model.JelimEntityModel
	var datas []model.JelimDetailEntity

	if err := trxmanager.New(s.Db).WithTrx(ctx, func(ctx *abstraction.Context) error {

		var getAje model.JelimFilterModel
		getAje.CompanyID = &payload.CompanyID
		getAje.Period = &payload.Period
		getAje.ConsolidationID = &payload.ConsolidationID
		

		getJelim, err := s.Repository.Get(ctx, &getAje)
		if err != nil {
			fmt.Println(err)
			return err
		}
		...
		data.Context = ctx
		data.JelimEntity = payload.JelimEntity
		data.JelimEntity.Status = 1
		// data.JelimEntity.Period = lastOfMonth.Format("2006-01-02")
		data.JelimEntity.TrxNumber = "JELIM" + "#" + strconv.Itoa(len(*getJelim)+1)
		...
		//create JelimDetail
		datas = payload.JelimDetail
		var arrJelimDetail []model.JelimDetailEntity
		for _, v := range datas {
			JelimDetail := model.JelimDetailEntityModel{
				Context:           ctx,
				JelimDetailEntity: v,
			}
			JelimDetail.JelimID = data.ID
			JelimDetail.ReffNumber = &data.TrxNumber

			_, err := s.JelimDetailRepository.Create(ctx, &JelimDetail)
			if err != nil {
				return err
			}
			findCoa, err := s.Repository.FindByCoa(ctx, &JelimDetail.CoaCode)
			if err != nil {
				return response.CustomErrorBuilder(http.StatusBadRequest, "Tidak Dapat Menemukan Coa "+JelimDetail.CoaCode, "Tidak Dapat Menemukan Coa "+JelimDetail.CoaCode)
			}

			findCoaType, err := s.Repository.FindByCoaType(ctx, &findCoa.CoaTypeID)
			if err != nil {
				return response.CustomErrorBuilder(http.StatusBadRequest, "Tidak Dapat Menemukan Coa di Type Coa "+JelimDetail.CoaCode, "Tidak Dapat Menemukan Coa di Type Coa "+JelimDetail.CoaCode)
			}
			findCoaGroup, err := s.Repository.FindByCoaGroup(ctx, &findCoaType.CoaGroupID)
			if err != nil {
				return response.CustomErrorBuilder(http.StatusBadRequest, "Tidak Dapat Menemukan Coa di Group coa "+JelimDetail.CoaCode, "Tidak Dapat Menemukan Coa di Group coa "+JelimDetail.CoaCode)
			}

			if findCoaGroup.Name == "ASET" {
				//  if *JelimDetail.BalanceSheetDr == float64(0) {
				...
				//  }
				//  if *JelimDetail.BalanceSheetCr != float64(0) {
				...
				//  }
				//  if *JelimDetail.IncomeStatementCr != float64(0) {
				...
				//  }
				//  if *JelimDetail.IncomeStatementDr != float64(0) {
				...
				tbID, err := s.Repository.FindByTbd(ctx, &data.ConsolidationID, &JelimDetail.CoaCode)
				if err != nil {
					return response.CustomErrorBuilder(http.StatusBadRequest, "Tidak Dapat Menemukan Coa di Kombinasi Konsolidasi "+JelimDetail.CoaCode, "Tidak Dapat Menemukan Coa "+JelimDetail.CoaCode)
				}
				tbID.Context = ctx

				AmountAjeCr := *v.BalanceSheetCr + *tbID.ConsolidationDetailEntity.AmountJelimCr
				AmountJelimDr := *v.BalanceSheetDr + *tbID.ConsolidationDetailEntity.AmountJelimDr
				tbID.ConsolidationDetailEntity.AmountJelimCr = &AmountAjeCr
				tbID.ConsolidationDetailEntity.AmountJelimDr = &AmountJelimDr
				...
				arrJelimDetail = append(arrJelimDetail, v)
			}
			if findCoaGroup.Name == "Liabilitas" {
				...
				AmountJelimCr := *v.BalanceSheetCr + *tbID.ConsolidationDetailEntity.AmountJelimCr
				AmountAjeDr := *v.BalanceSheetDr + *tbID.ConsolidationDetailEntity.AmountJelimDr
				tbID.ConsolidationDetailEntity.AmountJelimCr = &AmountJelimCr
				tbID.ConsolidationDetailEntity.AmountJelimDr = &AmountAjeDr
			...
			if findCoaGroup.Name == "Pendapatan" {
				...
				AmountJelimCr := *v.IncomeStatementCr + *tbID.ConsolidationDetailEntity.AmountJelimCr
				AmountAjeDr := *v.IncomeStatementDr + *tbID.ConsolidationDetailEntity.AmountJelimDr
				tbID.ConsolidationDetailEntity.AmountJelimCr = &AmountJelimCr
				tbID.ConsolidationDetailEntity.AmountJelimDr = &AmountAjeDr
			...
			if findCoaGroup.Name == "HPP/COGS" {
				...
				AmountAjeCr := *v.IncomeStatementCr + *tbID.ConsolidationDetailEntity.AmountJelimCr
				AmountAjeDr := *v.IncomeStatementDr + *tbID.ConsolidationDetailEntity.AmountJelimDr
				tbID.ConsolidationDetailEntity.AmountJelimCr = &AmountAjeCr
				tbID.ConsolidationDetailEntity.AmountJelimDr = &AmountAjeDr
		...
		for _, v := range *formatterDetailSumData {
			...
			if v.IsTotal != nil && *v.IsTotal == true && v.FxSummary != "" {
				...
				for _, tipe := range tmpString {
					...
					for _, vMatch := range match {
						...
						sumTBD, err := s.JpmRepository.FindSummarys(ctx, &vMatch, &result.ConsolidationID, v.IsCoa)
						if err != nil {
			...
			if v.Code == "LABA_BERSIH" {
				...
				customRowOne, err := s.JpmRepository.FindByTbd(ctx, &result.ConsolidationID, &code)
				if err != nil {
				...
				code = "310402002"
				criteriaTBDetail.Code = &code
				customRowTwo, err := s.JpmRepository.FindByTbd(ctx, &result.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "310501002"
				criteriaTBDetail.Code = &code
				customRowThree, err := s.JpmRepository.FindByTbd(ctx, &result.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "310502002"
				criteriaTBDetail.Code = &code
				customRowFour, err := s.JpmRepository.FindByTbd(ctx, &result.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "310503002"
				criteriaTBDetail.Code = &code
				customRowFive, err := s.JpmRepository.FindByTbd(ctx, &result.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				criteriaTBDetail.Code = &code
				dataReFa, err := s.JpmRepository.FindByTbd(ctx, &result.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				criteriaTBDetail.Code = &code
				dataFinIn, err := s.JpmRepository.FindByTbd(ctx, &result.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				criteriaTBDetail.Code = &code
				dataIncomeTax, err := s.JpmRepository.FindByTbd(ctx, &result.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				criteriaTBDetail.Code = &code
				dataForex, err := s.JpmRepository.FindByTbd(ctx, &result.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				criteriaTBDetail.Code = &code
				dataIncomeTax2, err := s.JpmRepository.FindByTbd(ctx, &result.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "LABA_BERSIH"
				criteriaTBDetail.Code = &code
				dataLaba, err := s.JpmRepository.FindByTbd(ctx, &result.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				criteriaTBDetail.Code = &code
				dataKomprehensif, err := s.JpmRepository.FindByTbd(ctx, &result.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}

				summaryCodes, err := s.JpmRepository.SummaryByCodes(ctx, &result.ConsolidationID, []string{"310501002", "310502002", "310503002"})
				if err != nil {
					return helper.ErrorHandler(err)
				}
		...
		for _, v := range *formatterDetailSumData {
			...
			if v.AutoSummary != nil && *v.AutoSummary {
				...
				for _, a := range *mfadetailsum {
					sumTBD, err := s.JpmRepository.FindSummarys(ctx, &v.Code, &result.ConsolidationID, v.IsCoa)
					if err != nil {
		...
		return &dto.JelimCreateResponse{}, err
	}
	result := &dto.JelimCreateResponse{
		JelimEntityModel: data,
	}
	return result, nil
}
func (s *service) Update(ctx *abstraction.Context, payload *dto.JelimUpdateRequest) (*dto.JelimUpdateResponse, error) {
	var data model.JelimEntityModel
	...
	if err := trxmanager.New(s.Db).WithTrx(ctx, func(ctx *abstraction.Context) error {
		...
		return &dto.JelimUpdateResponse{}, err
	}
	result := &dto.JelimUpdateResponse{
		JelimEntityModel: data,
	}
	return result, nil
}
func (s *service) Delete(ctx *abstraction.Context, payload *dto.JelimDeleteRequest) (*dto.JelimDeleteResponse, error) {
	var data model.JelimEntityModel
	if err := trxmanager.New(s.Db).WithTrx(ctx, func(ctx *abstraction.Context) error {
		if _, err := s.Repository.FindByID(ctx, &payload.ID); err != nil {
			return response.ErrorBuilder(&response.ErrorConstant.BadRequest, err)
		}
		FindAjeDetail, err := s.JelimDetailRepository.FindWithAjeID(ctx, &payload.ID)
		if err != nil {
			return err
		}
		JelimID, err := s.Repository.FindByID(ctx, &payload.ID)
		if err != nil {
			return err
		}
		var arrJelimDetails []model.JelimDetailEntityModel
		for _, v := range *FindAjeDetail {
			...
			// tbID, err := s.Repository.FindByTbd(ctx, &JelimID.ConsolidationID, &v.CoaCode )
			// if err != nil {
			// 	return err
			// }
			...
			// tbID.ConsolidationDetailEntity.AmountJelimCr = &AmountAjeCr
			// tbID.ConsolidationDetailEntity.AmountJelimDr = &AmountAjeDr
			// AmountConsole := +*tbID.AmountCombineSubsidiary + *tbID.AmountJelimDr - *tbID.AmountJelimDr
			// tbID.AmountConsole = &AmountConsole
			...
			// 	return err
			// }
			// arrJelimDetails = append(arrJelimDetails, JelimDetail)
			...
			if findCoaGroup.Name == "ASET" {
				...
				tbID, err := s.Repository.FindByTbd(ctx, &data.ConsolidationID, &v.CoaCode)
				if err != nil {
				...
				AmountAjeCr := *tbID.ConsolidationDetailEntity.AmountJelimCr - *v.BalanceSheetCr
				AmountJelimDr := *tbID.ConsolidationDetailEntity.AmountJelimDr - *v.BalanceSheetDr
				tbID.ConsolidationDetailEntity.AmountJelimCr = &AmountAjeCr
				tbID.ConsolidationDetailEntity.AmountJelimDr = &AmountJelimDr
				...
				arrJelimDetails = append(arrJelimDetails, v)
			}
			if findCoaGroup.Name == "Liabilitas" {
				//  if *JelimDetail.BalanceSheetDr != float64(0) {
				//  	return response.CustomErrorBuilder(http.StatusBadRequest, "Tidak Dapat Masukan Balance Sheet Debit", "Tidak Dapat Masukan Balance Sheet Debit")
				...
				AmountJelimCr := *tbID.ConsolidationDetailEntity.AmountJelimCr - *v.BalanceSheetCr
				AmountAjeDr := *tbID.ConsolidationDetailEntity.AmountJelimDr - *v.BalanceSheetDr
				tbID.ConsolidationDetailEntity.AmountJelimCr = &AmountJelimCr
				tbID.ConsolidationDetailEntity.AmountJelimDr = &AmountAjeDr
			...
			if findCoaGroup.Name == "Pendapatan" {
				...
				AmountJelimCr := *tbID.ConsolidationDetailEntity.AmountJelimCr - *v.IncomeStatementCr
				AmountAjeDr := *tbID.ConsolidationDetailEntity.AmountJelimDr - *v.IncomeStatementDr
				tbID.ConsolidationDetailEntity.AmountJelimCr = &AmountJelimCr
				tbID.ConsolidationDetailEntity.AmountJelimDr = &AmountAjeDr
			...
			if findCoaGroup.Name == "HPP/COGS" {
				...
				AmountAjeCr := *tbID.ConsolidationDetailEntity.AmountJelimCr - *v.IncomeStatementCr
				AmountAjeDr := *tbID.ConsolidationDetailEntity.AmountJelimDr - *v.IncomeStatementDr
				tbID.ConsolidationDetailEntity.AmountJelimCr = &AmountAjeCr
				tbID.ConsolidationDetailEntity.AmountJelimDr = &AmountAjeDr
		...
		for _, v := range *formatterDetailSumData {
			criteriaTBDetail := model.ConsolidationDetailFilterModel{}
			criteriaTBDetail.ConsolidationID = &JelimID.ConsolidationID
			if v.AutoSummary != nil && *v.AutoSummary {
				code := fmt.Sprintf("%s_Subtotal", v.Code)
				criteriaTBDetail.Code = &code
				criteriaTBDetail.ConsolidationID = &JelimID.ConsolidationID
				...
				for _, a := range *mfadetailsum {
					sumTBD, err := s.JpmRepository.FindSummarys(ctx, &v.Code, &JelimID.ConsolidationID, v.IsCoa)
					if err != nil {
						return response.CustomErrorBuilder(http.StatusBadRequest, "Gagal Menjumlahkan Summary Coa", "Gagal Menjumlahkan Summary Coa")
					}
			...
			if v.IsTotal != nil && *v.IsTotal == true && v.FxSummary != "" {
				...
				for _, tipe := range tmpString {
					...
					for _, vMatch := range match {
						...
						sumTBD, err := s.JpmRepository.FindSummarys(ctx, &vMatch, &JelimID.ConsolidationID, v.IsCoa)
						if err != nil {
							return response.CustomErrorBuilder(http.StatusBadRequest, "Gagal Mendapatkan Jumlah", "Gagal Mendapatkan Jumlah")
						}
						angka := 0.0
					...
					JelimID, err := expressionFormula.Evaluate(amountBeforeJpm)
					if err != nil {
						return response.CustomErrorBuilder(http.StatusBadRequest, "Tidak Dapat Masukan Balance Sheet Debit", "Tidak Dapat Masukan Balance Sheet Debit")
					}
					tmp := JelimID.(float64)
			...
			if v.Code == "LABA_BERSIH" {
				...
				criteriaTBDetail.ConsolidationID = &JelimID.ConsolidationID
				criteriaTBDetail.Code = &code
				customRowOne, err := s.JpmRepository.FindByTbd(ctx, &JelimID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "310402002"
				criteriaTBDetail.Code = &code
				customRowTwo, err := s.JpmRepository.FindByTbd(ctx, &JelimID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "310501002"
				criteriaTBDetail.Code = &code
				customRowThree, err := s.JpmRepository.FindByTbd(ctx, &JelimID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "310502002"
				criteriaTBDetail.Code = &code
				customRowFour, err := s.JpmRepository.FindByTbd(ctx, &JelimID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "310503002"
				criteriaTBDetail.Code = &code
				customRowFive, err := s.JpmRepository.FindByTbd(ctx, &JelimID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "950101001" //REVALUATION FA (4337)
				criteriaTBDetail.Code = &code
				dataReFa, err := s.JpmRepository.FindByTbd(ctx, &JelimID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "950301001" //Financial Instrument (4342)
				criteriaTBDetail.Code = &code
				dataFinIn, err := s.JpmRepository.FindByTbd(ctx, &JelimID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "950301002" //Income tax relating to components of OCI (4343)
				criteriaTBDetail.Code = &code
				dataIncomeTax, err := s.JpmRepository.FindByTbd(ctx, &JelimID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "950401001" //Foreign Exchange (4345)
				criteriaTBDetail.Code = &code
				dataForex, err := s.JpmRepository.FindByTbd(ctx, &JelimID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "950401002" //Income tax relating to components of OCI (4346)
				criteriaTBDetail.Code = &code
				dataIncomeTax2, err := s.JpmRepository.FindByTbd(ctx, &JelimID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "LABA_BERSIH"
				criteriaTBDetail.Code = &code
				dataLaba, err := s.JpmRepository.FindByTbd(ctx, &JelimID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "TOTAL_PENGHASILAN_KOMPREHENSIF_LAIN~BS"
				criteriaTBDetail.Code = &code
				dataKomprehensif, err := s.JpmRepository.FindByTbd(ctx, &JelimID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}

				summaryCodes, err := s.JpmRepository.SummaryByCodes(ctx, &JelimID.ConsolidationID, []string{"310501002", "310502002", "310503002"})
				if err != nil {
					return helper.ErrorHandler(err)
				}
		...
		return &dto.JelimDeleteResponse{}, err
	}
	result := &dto.JelimDeleteResponse{
		JelimEntityModel: data,
	}
	return result, nil
}
...
func (s *service) Export(ctx *abstraction.Context, payload *dto.JelimExportRequest) (*dto.JelimExportResponse, error) {

	datas, err := s.Repository.Export(ctx, &payload.JelimID)
	if err != nil {
		return &dto.JelimExportResponse{}, response.ErrorBuilder(&response.ErrorConstant.InternalServerError, err)
	}
	...
	fileName := fmt.Sprintf("Jurnal_ELIM_%s.xlsx", period)
	...
	result := &dto.JelimExportResponse{
		FileName: fileName,
		Path:     fileLoc,
	}
	return result, nil
