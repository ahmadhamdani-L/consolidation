...
type Service interface {
	Find(ctx *abstraction.Context, payload *dto.JcteGetRequest) (*dto.JcteGetResponse, error)
	FindByID(ctx *abstraction.Context, payload *dto.JcteGetByIDRequest) (*dto.JcteGetByIDResponse, error)
	Create(ctx *abstraction.Context, payload *dto.JcteCreateRequest) (*dto.JcteCreateResponse, error)
	Update(ctx *abstraction.Context, payload *dto.JcteUpdateRequest) (*dto.JcteUpdateResponse, error)
	Delete(ctx *abstraction.Context, payload *dto.JcteDeleteRequest) (*dto.JcteDeleteResponse, error)
	Export(ctx *abstraction.Context, payload *dto.JcteExportRequest) (*dto.JcteExportResponse, error)
	GetVersion(ctx *abstraction.Context, payload *dto.GetVersionRequest) (*dto.GetVersionResponse, error)
}

func NewService(f *factory.Factory) *service {
	repository := f.JcteRepository
	JcteDetailRepository := f.JcteDetailRepository
	JpmRepository := f.JpmRepository

	db := f.Db
	return &service{
		Repository:           repository,
		JcteDetailRepository: JcteDetailRepository,
		JpmRepository:        JpmRepository,
		Db:                   db,
	}
}

func (s *service) Find(ctx *abstraction.Context, payload *dto.JcteGetRequest) (*dto.JcteGetResponse, error) {
	data, info, err := s.Repository.Find(ctx, &payload.JcteFilterModel, &payload.Pagination)
	if err != nil {
		return &dto.JcteGetResponse{}, response.ErrorBuilder(&response.ErrorConstant.InternalServerError, err)
	}
	result := &dto.JcteGetResponse{
		Datas:          *data,
		PaginationInfo: *info,
	}
	return result, nil
...
func (s *service) FindByID(ctx *abstraction.Context, payload *dto.JcteGetByIDRequest) (*dto.JcteGetByIDResponse, error) {
	data, err := s.Repository.FindByID(ctx, &payload.ID)
	if err != nil {
		return &dto.JcteGetByIDResponse{}, response.ErrorBuilder(&response.ErrorConstant.InternalServerError, err)
	}
	result := &dto.JcteGetByIDResponse{
		JcteEntityModel: *data,
	}
	return result, nil
}
func (s *service) Create(ctx *abstraction.Context, payload *dto.JcteCreateRequest) (*dto.JcteCreateResponse, error) {
	var data model.JcteEntityModel
	var datas []model.JcteDetailEntity

	if payload.JcteEntity.ConsolidationID == 0 {
		return nil, response.CustomErrorBuilder(http.StatusBadRequest, "Consolidation ID Harus Di isi", "Consolidation ID Harus Di isi")
	}
	if err := trxmanager.New(s.Db).WithTrx(ctx, func(ctx *abstraction.Context) error {
		fmt.Println(err)
		
		var getAje model.JcteFilterModel
		getAje.CompanyID = &payload.CompanyID
		getAje.Period = &payload.Period
		getAje.ConsolidationID = &payload.ConsolidationID
		

		getJcte, err := s.Repository.Get(ctx, &getAje)
		if err != nil {
			fmt.Println(err)
			return err
		}
		...
		data.Context = ctx
		data.JcteEntity = payload.JcteEntity
		data.JcteEntity.Status = 1
		// data.JcteEntity.Period = lastOfMonth.Format("2006-01-02")
		data.JcteEntity.TrxNumber = "JCTE" + "#" + strconv.Itoa(len(*getJcte)+1)
		...
		//create JcteDetail
		datas = payload.JcteDetail
		var arrJcteDetail []model.JcteDetailEntity
		for _, v := range datas {
			JcteDetail := model.JcteDetailEntityModel{
				Context:          ctx,
				JcteDetailEntity: v,
			}
			JcteDetail.JcteID = data.ID
			JcteDetail.ReffNumber = &data.TrxNumber

			_, err := s.JcteDetailRepository.Create(ctx, &JcteDetail)
			if err != nil {
				return err
			}

			findCoa, err := s.Repository.FindByCoa(ctx, &JcteDetail.CoaCode)
			if err != nil {
				return response.CustomErrorBuilder(http.StatusBadRequest, "Tidak Dapat Menemukan Coa "+JcteDetail.CoaCode, "Tidak Dapat Menemukan Coa "+JcteDetail.CoaCode)
			}

			findCoaType, err := s.Repository.FindByCoaType(ctx, &findCoa.CoaTypeID)
			if err != nil {
				return response.CustomErrorBuilder(http.StatusBadRequest, "Tidak Dapat Menemukan Coa di Type Coa "+JcteDetail.CoaCode, "Tidak Dapat Menemukan Coa di Type Coa "+JcteDetail.CoaCode)
			}
			findCoaGroup, err := s.Repository.FindByCoaGroup(ctx, &findCoaType.CoaGroupID)
			if err != nil {
				return response.CustomErrorBuilder(http.StatusBadRequest, "Tidak Dapat Menemukan Coa di Group coa "+JcteDetail.CoaCode, "Tidak Dapat Menemukan Coa di Group coa "+JcteDetail.CoaCode)
			}

			if findCoaGroup.Name == "ASET" {

				tbID, err := s.Repository.FindByTbd(ctx, &data.ConsolidationID, &JcteDetail.CoaCode)
				if err != nil {
					return response.CustomErrorBuilder(http.StatusBadRequest, "Tidak Dapat Menemukan Coa di Kombinasi Konsolidasi "+JcteDetail.CoaCode, "Tidak Dapat Menemukan Coa "+JcteDetail.CoaCode)
				}
				tbID.Context = ctx

				AmountAjeCr := *v.BalanceSheetCr + *tbID.ConsolidationDetailEntity.AmountJcteCr
				AmountAjeDr := *v.BalanceSheetDr + *tbID.ConsolidationDetailEntity.AmountJcteDr
				tbID.ConsolidationDetailEntity.AmountJcteCr = &AmountAjeCr
				...
				arrJcteDetail = append(arrJcteDetail, v)
			}
			...
			if findCoaGroup.Name == "Pendapatan" {
				...
				AmountJcteCr := *v.IncomeStatementCr + *tbID.ConsolidationDetailEntity.AmountJcteCr
				AmountAjeDr := *v.IncomeStatementDr + *tbID.ConsolidationDetailEntity.AmountJcteDr
				tbID.ConsolidationDetailEntity.AmountJcteCr = &AmountJcteCr
				tbID.ConsolidationDetailEntity.AmountJcteDr = &AmountAjeDr
				AmountAfterJcte := *tbID.AmountAfterJpm - *tbID.AmountJcteDr + *tbID.AmountJcteCr
				tbID.AmountAfterJcte = &AmountAfterJcte
			...
			if findCoaGroup.Name == "HPP/COGS" {
				// if *JcteDetail.BalanceSheetDr != float64(0) {
				...
				// }
				// if *JcteDetail.BalanceSheetCr != float64(0) {
				...
				// }
				// if *JcteDetail.IncomeStatementCr != float64(0) {
				...
				// }
				// if *JcteDetail.IncomeStatementDr == float64(0) {
				...
				AmountAjeCr := *v.IncomeStatementCr + *tbID.ConsolidationDetailEntity.AmountJcteCr
				AmountAjeDr := *v.IncomeStatementDr + *tbID.ConsolidationDetailEntity.AmountJcteDr
				tbID.ConsolidationDetailEntity.AmountJcteCr = &AmountAjeCr
				tbID.ConsolidationDetailEntity.AmountJcteDr = &AmountAjeDr
				AmountAfterJcte := *tbID.AmountAfterJpm + *tbID.AmountJcteDr - *tbID.AmountJcteCr
			...
			if findCoaGroup.Name == "Other Expense" {
				...
				AmountAjeCr := *v.IncomeStatementDr + *tbID.ConsolidationDetailEntity.AmountJcteCr
				AmountAjeDr := *v.IncomeStatementDr + *tbID.ConsolidationDetailEntity.AmountJcteDr
				tbID.ConsolidationDetailEntity.AmountJcteCr = &AmountAjeCr
				tbID.ConsolidationDetailEntity.AmountJcteDr = &AmountAjeDr
				AmountAfterJcte := *tbID.AmountAfterJpm + *tbID.AmountJcteDr - *tbID.AmountJcteCr
		...
		return &dto.JcteCreateResponse{}, err
	}
	result := &dto.JcteCreateResponse{
		JcteEntityModel: data,
	}
	return result, nil
}
func (s *service) Update(ctx *abstraction.Context, payload *dto.JcteUpdateRequest) (*dto.JcteUpdateResponse, error) {
	var data model.JcteEntityModel
	...
	if err := trxmanager.New(s.Db).WithTrx(ctx, func(ctx *abstraction.Context) error {
		...
		return &dto.JcteUpdateResponse{}, err
	}
	result := &dto.JcteUpdateResponse{
		JcteEntityModel: data,
	}
	return result, nil
}
func (s *service) Delete(ctx *abstraction.Context, payload *dto.JcteDeleteRequest) (*dto.JcteDeleteResponse, error) {
	var data model.JcteEntityModel
	if err := trxmanager.New(s.Db).WithTrx(ctx, func(ctx *abstraction.Context) error {
		if _, err := s.Repository.FindByID(ctx, &payload.ID); err != nil {
			return response.ErrorBuilder(&response.ErrorConstant.BadRequest, err)
		}
		FindAjeDetail, err := s.JcteDetailRepository.FindWithAjeID(ctx, &payload.ID)
		if err != nil {
			return err
		}
		JcteID, err := s.Repository.FindByID(ctx, &payload.ID)
		if err != nil {
			return err
		}
		var arrJcteDetail []model.JcteDetailEntityModel
		for _, v := range *FindAjeDetail {
			...
			result, err := s.Repository.FindByID(ctx, &payload.ID)
			if err != nil {
				return err
			}
			...
			if findCoaGroup.Name == "ASET" {
				...
				tbID, err := s.Repository.FindByTbd(ctx, &result.ConsolidationID, &v.CoaCode)
				if err != nil {
				...
				AmountAjeCr := *tbID.ConsolidationDetailEntity.AmountJcteCr - *v.BalanceSheetCr
				AmountAjeDr := *tbID.ConsolidationDetailEntity.AmountJcteDr - *v.BalanceSheetDr
				tbID.ConsolidationDetailEntity.AmountJcteCr = &AmountAjeCr
				tbID.ConsolidationDetailEntity.AmountJcteDr = &AmountAjeDr
				AmountAfterJcte := *tbID.AmountAfterJpm + *tbID.AmountJcteDr - *tbID.AmountJcteCr
				tbID.AmountAfterJcte = &AmountAfterJcte
			...
			if findCoaGroup.Name == "Pendapatan" {
				...
				AmountJcteCr := *tbID.ConsolidationDetailEntity.AmountJcteCr - *v.IncomeStatementCr
				AmountAjeDr := *tbID.ConsolidationDetailEntity.AmountJcteDr - *v.IncomeStatementDr
				tbID.ConsolidationDetailEntity.AmountJcteCr = &AmountJcteCr
				tbID.ConsolidationDetailEntity.AmountJcteDr = &AmountAjeDr
				AmountAfterJcte := *tbID.AmountAfterJpm - *tbID.AmountJcteDr + *tbID.AmountJcteCr
				tbID.AmountAfterJcte = &AmountAfterJcte
		...
		for _, v := range *formatterDetailSumData {
			criteriaTBDetail := model.ConsolidationDetailFilterModel{}
			criteriaTBDetail.ConsolidationID = &JcteID.ConsolidationID
			if v.AutoSummary != nil && *v.AutoSummary {
				code := fmt.Sprintf("%s_Subtotal", v.Code)
				criteriaTBDetail.Code = &code
				criteriaTBDetail.ConsolidationID = &JcteID.ConsolidationID
				...
				for _, a := range *mfadetailsum {
					sumTBD, err := s.JpmRepository.FindSummarys(ctx, &v.Code, &JcteID.ConsolidationID, v.IsCoa)
					if err != nil {
						return response.CustomErrorBuilder(http.StatusBadRequest, "Gagal Menjumlahkan Summary Coa", "Gagal Menjumlahkan Summary Coa")
					}
			...
			if v.IsTotal != nil && *v.IsTotal == true && v.FxSummary != "" {
				...
				for _, tipe := range tmpString {
					...
					for _, vMatch := range match {
						...
						sumTBD, err := s.JpmRepository.FindSummarys(ctx, &vMatch, &JcteID.ConsolidationID, v.IsCoa)
						if err != nil {
							return response.CustomErrorBuilder(http.StatusBadRequest, "Gagal Mendapatkan Jumlah", "Gagal Mendapatkan Jumlah")
						}
						angka := 0.0
			...
			if v.Code == "LABA_BERSIH" {
				...
				criteriaTBDetail.ConsolidationID = &JcteID.ConsolidationID
				criteriaTBDetail.Code = &code
				customRowOne, err := s.Repository.FindByTbd(ctx, &JcteID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "310402002"
				criteriaTBDetail.Code = &code
				customRowTwo, err := s.Repository.FindByTbd(ctx, &JcteID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "310501002"
				criteriaTBDetail.Code = &code
				customRowThree, err := s.Repository.FindByTbd(ctx, &JcteID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "310502002"
				criteriaTBDetail.Code = &code
				customRowFour, err := s.Repository.FindByTbd(ctx, &JcteID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "310503002"
				criteriaTBDetail.Code = &code
				customRowFive, err := s.Repository.FindByTbd(ctx, &JcteID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "950101001" //REVALUATION FA (4337)
				criteriaTBDetail.Code = &code
				dataReFa, err := s.Repository.FindByTbd(ctx, &JcteID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "950301001" //Financial Instrument (4342)
				criteriaTBDetail.Code = &code
				dataFinIn, err := s.Repository.FindByTbd(ctx, &JcteID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "950301002" //Income tax relating to components of OCI (4343)
				criteriaTBDetail.Code = &code
				dataIncomeTax, err := s.Repository.FindByTbd(ctx, &JcteID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "950401001" //Foreign Exchange (4345)
				criteriaTBDetail.Code = &code
				dataForex, err := s.Repository.FindByTbd(ctx, &JcteID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "950401002" //Income tax relating to components of OCI (4346)
				criteriaTBDetail.Code = &code
				dataIncomeTax2, err := s.Repository.FindByTbd(ctx, &JcteID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "LABA_BERSIH"
				criteriaTBDetail.Code = &code
				dataLaba, err := s.Repository.FindByTbd(ctx, &JcteID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}
				...
				code = "TOTAL_PENGHASILAN_KOMPREHENSIF_LAIN~BS"
				criteriaTBDetail.Code = &code
				dataKomprehensif, err := s.Repository.FindByTbd(ctx, &JcteID.ConsolidationID, &code)
				if err != nil {
					return helper.ErrorHandler(err)
				}

				summaryCodes, err := s.JpmRepository.SummaryByCodes(ctx, &JcteID.ConsolidationID, []string{"310501002", "310502002", "310503002"})
				if err != nil {
					return helper.ErrorHandler(err)
				}
		...
		return &dto.JcteDeleteResponse{}, err
	}
	result := &dto.JcteDeleteResponse{
		JcteEntityModel: data,
	}
	return result, nil
}
...
func (s *service) Export(ctx *abstraction.Context, payload *dto.JcteExportRequest) (*dto.JcteExportResponse, error) {

	datas, err := s.Repository.Export(ctx, &payload.JcteID)
	if err != nil {
		return &dto.JcteExportResponse{}, response.ErrorBuilder(&response.ErrorConstant.InternalServerError, err)
	}
	...
	fileName := fmt.Sprintf("Jurnal_CTE_%s.xlsx", period)
