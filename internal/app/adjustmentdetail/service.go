...
func (s *service) Find(ctx *abstraction.Context, payload *dto.AdjustmentDetailGetRequest) (*dto.AdjustmentDetailGetResponse, error) {
	data, info, err := s.Repository.Find(ctx, &payload.AdjustmentDetailFilterModel, &payload.Pagination)
	if err != nil {
		return &dto.AdjustmentDetailGetResponse{}, response.ErrorBuilder(&response.ErrorConstant.InternalServerError, err)
	}
...
func (s *service) FindByID(ctx *abstraction.Context, payload *dto.AdjustmentDetailGetByIDRequest) (*dto.AdjustmentDetailGetByIDResponse, error) {
	...
	if err != nil {
		return &dto.AdjustmentDetailGetByIDResponse{}, response.ErrorBuilder(&response.ErrorConstant.InternalServerError, err)
	}
...
func (s *service) Create(ctx *abstraction.Context, payload *dto.AdjustmentDetailCreateRequest) (*dto.AdjustmentDetailCreateResponse, error) {
	...
	if err := trxmanager.New(s.Db).WithTrx(ctx, func(ctx *abstraction.Context) error {
		...
		AdjustmentID, err := s.AdjustmentRepository.FindByID(ctx, &payload.ID)
		if err != nil {
			return err
		}
		source := "TRIAL-BALANCE"
		fID, err := s.AdjustmentRepository.FindByFormatter(ctx, &AdjustmentID.TbID, &source)
		if err != nil {
			return err
		}
		var arrAdjustmentDetails []model.AdjustmentDetailEntityModel
		for _, v := range *FindAjeDetail {
			...
			// tbID, err := s.AdjustmentRepository.FindByTbd(ctx, &fID.ID, &v.CoaCode)
			// if err != nil {
			...
			// }
			// _, err = s.AdjustmentRepository.UpdateTbd(ctx, &tbID.ID, tbID)
			// if err != nil {
			// 	return err
			// }
			...
			if findCoaGroup.Name == "ASET" {
				...
				arrAdjustmentDetails = append(arrAdjustmentDetails, v)
			}
		...
		for _, v := range datas {
			...
			_, err := s.Repository.Create(ctx, &AdjustmentDetail)
			...
			if findCoaGroup.Name == "ASET" {
				...
				tbID, err := s.AdjustmentRepository.FindByTbd(ctx, &fID.ID, &AdjustmentDetail.CoaCode)
				if err != nil {
					return response.CustomErrorBuilder(http.StatusBadRequest, "Tidak Dapat Menemukan Coa di Kombinasi Konsolidasi "+AdjustmentDetail.CoaCode, "Tidak Dapat Menemukan Coa "+AdjustmentDetail.CoaCode)
				}
...
func (s *service) Update(ctx *abstraction.Context, payload *dto.AdjustmentDetailUpdateRequest) (*dto.AdjustmentDetailUpdateResponse, error) {
	...
	if err := trxmanager.New(s.Db).WithTrx(ctx, func(ctx *abstraction.Context) error {
		AId, err := s.Repository.FindByID(ctx, &payload.ID)
		if err != nil {
			return response.ErrorBuilder(&response.ErrorConstant.InternalServerError, err)
		}
		data1 := *AId
		...
		data = *result
		source := "TRIAL-BALANCE"
		fID, err := s.AdjustmentRepository.FindByFormatter(ctx, &data.Adjustment.TbID, &source)
		if err != nil {
			return err
		}

		tbID, err := s.AdjustmentRepository.FindByTbd(ctx, &fID.ID, &data.CoaCode)
		if err != nil {
			return response.CustomErrorBuilder(http.StatusBadRequest, "Tidak Dapat Menemukan Coa "+data.CoaCode, "Tidak Dapat Menemukan Coa "+data.CoaCode)
		}
		tbID.Context = ctx

		AmountAjeCr := *tbID.TrialBalanceDetailEntity.AmountAjeCr - *data1.BalanceSheetCr + *data.BalanceSheetCr
		AmountAjeDr := *tbID.TrialBalanceDetailEntity.AmountAjeDr - *data1.BalanceSheetDr + *data.BalanceSheetDr
		tbID.TrialBalanceDetailEntity.AmountAjeCr = &AmountAjeCr
		tbID.TrialBalanceDetailEntity.AmountAjeDr = &AmountAjeDr
		AmountAfterAje := +*tbID.AmountBeforeAje + *tbID.AmountAjeDr - *tbID.AmountAjeCr
		tbID.AmountAfterAje = &AmountAfterAje

		ReffAjeDr := *data.ReffNumber + "," + *tbID.ReffAjeDr
		if *data.BalanceSheetDr > float64(0) {
			tbID.ReffAjeDr = &ReffAjeDr
		}
		ReffAjeCr := *data.ReffNumber + "," + *tbID.ReffAjeCr
		if *data.BalanceSheetCr > float64(0) {
			tbID.ReffAjeCr = &ReffAjeCr
		}
		_, err = s.AdjustmentRepository.UpdateTbd(ctx, &tbID.ID, tbID)
		if err != nil {
		...
		return &dto.AdjustmentDetailUpdateResponse{}, err
	}
