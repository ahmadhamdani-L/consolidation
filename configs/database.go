...
func DB() *DbConfig {
	dbOnce.Do(func() {
		...
		strMaxOpenConns := PriorityString(fang.GetString("db.max_open_connections"), os.Getenv("DB_MAX_OPEN_CONNECTIONS"), "10")
		mOpenConns, err := strconv.Atoi(strMaxOpenConns)
		if err != nil {
			mOpenConns = 0
		}

		strMaxIdleConns := PriorityString(fang.GetString("db.max_idle_connections"), os.Getenv("DB_MAX_IDLE_CONNECTIONS"), "10")
		mIdleConns, err := strconv.Atoi(strMaxIdleConns)
		if err != nil {
			mIdleConns = 2
		}

		strConnLifetime := PriorityString(fang.GetString("db.connection_lifetime"), os.Getenv("DB_CONNECTION_LIFETIME"), fmt.Sprintf("%d", (time.Minute*1)))
		...
		db = &DbConfig{
			...
			name:         PriorityString(fang.GetString("db.name"), os.Getenv("DB_NAME"), "db_name"),
			user:         PriorityString(fang.GetString("db.user"), os.Getenv("DB_USER"), "postgres"),
			pass:         PriorityString(fang.GetString("db.pass"), os.Getenv("DB_PASS"), "postgres"),
