...
func DB() *DbConfig {
	dbOnce.Do(func() {
		...
		strMaxOpenConns := PriorityString(fang.GetString("db.max_open_connections"), os.Getenv("DB_MAX_OPEN_CONNECTIONS"), "12")
		mOpenConns, err := strconv.Atoi(strMaxOpenConns)
		if err != nil {
			mOpenConns = 10
		}
		fmt.Println(mOpenConns)

		strMaxIdleConns := PriorityString(fang.GetString("db.max_idle_connections"), os.Getenv("DB_MAX_IDLE_CONNECTIONS"), "2")
